...
const Pagination = (props) => {
  ...
  return (
    <div className={ccss.pageNavWrap}>
      ...
      {page == 1 ? null : (
        <Link href={{ query: setParams(page - 1) }} className={ccss.pageNum + " mr-2"} scroll={false}>
          ◀
        </Link>
      )}
      {/* 고정 1st btn */}
      {page == 1 ? (
        ...
        <Link href={{ query: setParams(1) }} className={ccss.pageNum} scroll={false}>
          1
        </Link>
      )}
      {/* 중간 페이지 */}
      {pageNums.map((pageNum) => {
        return pageNum == -1 ? (
          ...
          <Link href={{ query: setParams(pageNum) }} key={`btn${pageNum}`} className={ccss.pageNum} scroll={false}>
            {pageNum}
          </Link>
        );
      })}
      ...
      {maxPages < 2 ? null : page == maxPages ? (
        ...
        <Link href={{ query: setParams(maxPages) }} className={ccss.pageNum} scroll={false}>
          {maxPages}
        </Link>
      )}
      {/* 고정 next btn. 현재 last 페이지면 표시 안함 */}
      {page == maxPages ? null : (
        <Link href={{ query: setParams(page + 1) }} className={ccss.pageNum + " ml-2"} scroll={false}>
          ▶
        </Link>
      )}
    </div>
